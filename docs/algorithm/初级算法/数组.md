...
​		两两对比，如果相等（1.放入list，2.指针都往后走一步）
​		如果 a<b，则 a 的指针向后走一步对比，
​		如果 b<a，则 b 的指针向后走一步，再对比
​		没得走，while循环结束，返回数据
...
2. 如果是 9 ，则变为 0 ，继续循环加一，知道不是9就return
3. 如果没有return出去，说明都是9，那么需要进一位数 9999 -> 10000
...
2. 使用双指针，一个指针 L 指向 0，另一个 R 往前走
3. if L = 0, R !==0 交换，L++,R++
4. if L !==0 R!==0 ，一般出现一开始的，都往前
...
1. length >=2 了，边界情况
2. 哈希表，key value 对应, map[target - nums[i]] = i;
3. if map[nums[i]] 已存在，说明啊之前，已经有值可以和当前值配对成 target了
...
2. 比如 (1,1)坐标下，有个值是 5，那么 `row[1,5]` 、`col[1,5]`、`box[0][0][5]` 都加一
3. 一旦发现加一后，大于1 了，说明了存在重复，return false
...
你必须在 原地 旋转图像，这意味着你需要直接修改输入的二维矩阵。请不要 使用另一个矩阵来旋转图像。
 ...
 <img src="https://assets.leetcode.com/uploads/2020/08/28/mat1.jpg" alt="img" style="zoom:67%;" />      <img src="https://assets.leetcode.com/uploads/2020/08/28/mat2.jpg" alt="img" style="zoom: 67%;" />
...
1. 能够用语法快速创建数组
2. 数组使用哈希表需要学会用 Map 和 Set
3. 双指针的使用、for 、while、for of、
4. 有时可以先排好序，再求解
5. 使用结构存储子问题的解 - 动态规划
6. 贪心算法
