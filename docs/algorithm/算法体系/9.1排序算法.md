...
/*
* 1. 分治
* 2. 划分，左边比你小，右边比你大。从基准左区分，找到比自己小的，放到自己后面，找完后，把自己的和最后一个的位置交换， 这样左边都是比你小的，右边都是比你大的
* 3. 好了那就递归把
* 4. 直到了终结条件，返回这个数组
* */
const testArr = Array.from({length:50},val=>Math.ceil(Math.random()*100))
...
function partition(arr, left, right) {
  ...
  [arr[currIndex], arr[index - 1]] = [arr[index - 1], arr[currIndex]]
  return index - 1
}
